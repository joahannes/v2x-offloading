...
//-------------Rodar aplicação EvalVid
  ...
  server.SetAttribute ("SenderDumpFilename", StringValue("resultados/sd_a01"));
  ...
  //ClienteEvalvid
  EvalvidClientHelper client (i1.GetAddress (0), port);
  client.SetAttribute ("ReceiverDumpFilename", StringValue("resultados/rd_a01"));
  apps = client.Install (ueNodes.Get (numberOfUes - 1));
  apps.Start (Seconds (15.0));
  apps.Stop (Seconds (39.0));
